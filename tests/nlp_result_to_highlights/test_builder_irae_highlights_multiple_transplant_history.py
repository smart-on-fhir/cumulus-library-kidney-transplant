...
@pytest.fixture()
def builder():
    return builder_module.IraeNlpHighlightsMultipleTransplantHistoryBuilder()
...
def test_is_table_valid_delegates_to_sql_utils(database, builder):
    ...
    with mock.patch.object(
        ...
        result = builder._is_table_valid(database, "irae__nlp_multiple_transplant_history_gpt4o")

    assert result 
    mock_is_field_present.assert_called_once_with(
        database=database,
        source_table="irae__nlp_multiple_transplant_history_gpt4o",
        source_col="result",
        expected={},
)
...
def test_prepare_queries_appends_template_from_valid_tables(database, builder):
    ...
    rendered_query = "select * from irae__nlp_multiple_transplant_history_gpt5"
    ...
    mock_get_template.assert_called_once_with(
        "irae__llm_multiple_transplant_history_highlights",
